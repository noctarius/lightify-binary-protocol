...
local pf_packet_type        = ProtoField.uint8 ("lightify.packet_type", "Packet type", base.DEC, packet_types, 0xFF)
local pf_command_id         = ProtoField.uint8 ("lightify.command_id", "Command Id", base.DEC, command_ids, 0xFF)
local pf_request_id         = ProtoField.uint32("lightify.request_id", "Request Id")

local pf_response_code      = ProtoField.uint8 ("lightify.response_code", "Response code", base.DEC, response_codes, 0xFF)
...
local pf_device_type        = ProtoField.uint8 ("lightify.device_type", "Device Type", base.DEC, device_types)
...
local pf_reachable          = ProtoField.uint8 ("lightify.reachable", "Reachable", base.DEC, reachable_states)
...
lightify.fields = {
    ...
    pf_device_temperature, pf_device_color, pf_device_name, pf_transition_time, pf_zone_id, pf_zone_name, pf_reachable,
    pf_reserved,
    pf_number
}
    ...
    return {
        ...
        function(buffer, info, root)
            ...
            for i = 1, numDevices do
                ...
                deviceRoot:add(pf_device_name, buffer:range(pos + 26, 24), extract_name(buffer, pos + 26, 24))
                pos = pos + 50
            end
        end
    }
...
function device_address(buffer)
    ...
    return string.format('%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x', dst0, dst1, dst2, dst3, dst4, dst5, dst6, dst7)
end
